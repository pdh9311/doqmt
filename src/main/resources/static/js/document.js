...
const documentClick = (username, documentName, documentId) => () => {
  location.href = `http://3.39.248.143:8080/@${username}/doc/read?book=${bookId}&doc=${documentId}`;
};
...
const documentKeyup = (documentInput, documentId) => (e) => {
  if (e.keyCode === 13) {     // Enter
    // 문서명 변경 요청
    axios
        .patch(`http://3.39.248.143:8080/@${username}/doc/title/${documentId}`, {
...
const documentDelClick = (document, documentId) => () => {
  ...
  // 문서 삭제
  axios
      .patch(`http://3.39.248.143:8080/@${username}/doc/delete/${documentId}`)
      .then((response) => {
        const data = response.data;
      })
      .catch((error) => {
