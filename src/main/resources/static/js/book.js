...
const bookClick = (username, bookName, bookId) => () => {
  location.href = `http://doqmt.com/@${username}/docs?book=${bookId}`;
...
const bookKeyup = (bookInput, bookId) => (e) => {
  if (e.keyCode === 13) {     // Enter
    // 카테고리명 변경 요청
    axios
        .patch(`http://doqmt.com/@${username}/book/name/${bookId}`, {
...
const bookDelClick = (book, bookId) => () => {
  // 카테고리 삭제
  axios
      .patch(`http://doqmt.com/@${username}/book/delete/${bookId}`)
      .then((response) => {
        const data = response.data;
        if (response.data === 0) {
          axios
              .delete(`http://doqmt.com/@${username}/book/${bookId}`)
              .then((response) => {
                const data = response.data;
              })
              .catch((error) => {
...
if ($addbookName !== null) {
  ...
  $addbookName.addEventListener('keyup', (e) => {
    if (e.keyCode === 13 && $addbookName.value !== '') {
      // 카테고리 저장
      axios
          .post(`http://doqmt.com/@${username}/book/add`, {
